...
import { communityPostSms } from '../../../lib/sms-segments'
...
async function textCampersAboutStaffPost({
  admin,
  user,
  post,
  author,
  ...
  const { data: camperRows, error: camperError } = await admin
    ...
    .eq('sms_opt_in', true)
  if (camperError) {
    console.error('Community post text recipients could not be loaded:', camperError.message)
    return
  }
  ...
  if (!recipientPlan.recipients.length) return
  ...
  if (campaignError?.code === '23505') return
  if (campaignError || !campaign) {
    console.error('Community post text campaign could not be reserved:', campaignError?.message || 'Unknown error')
    return
  }
