...
const ONE_TIME_KEY = '670f7e3329358235b054b68ed2e5d382cbd6db8267d0a393'
const REMINDER_TYPE = 'Dinner Group Reminder 2026-08-29 6 PM'
const MESSAGE = camperTextWithLink({
  message: 'Reminder: Dinner is at 6:00 PM. Thank you!',
  path: '/dinners',
  linkLabel: 'Dinner details',
})
...
export async function POST(request: Request) {
  ...
  try {
    ...
    for (const camper of (campers || []).filter(isOperationalCamper)) {
      ...
      for (const phone of phones) {
        ...
        await admin.from('text_reminders').insert({
          ...
          sent_by: 'one-time-dinner-six-reminder-2026-08-29',
        })
    ...
    return NextResponse.json({ error: error?.message || 'Unable to send the dinner reminder.' }, { status: 500 })
  }
}
