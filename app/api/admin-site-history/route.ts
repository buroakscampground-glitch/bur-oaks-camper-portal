...
export async function GET(request: Request) {
  ...
  const [camperResult, invoiceResult, noticeResult, maintenanceResult, pumpResult, documentResult, messageResult, readingResult] = await Promise.all([
    ...
    context.admin.from('documents').select('id,document_name,document_type,signature_status,signed_at').eq('camper_id', camperId).order('document_name', { ascending: true }).limit(100),
  ...
  const activity = [
    ...
    ...(documentResult.data || []).map((item) => ({ id: `document-${item.id}`, type: 'Document', title: item.document_name || item.document_type || 'Document', detail: item.signature_status || 'Pending', date: item.signed_at || null })),
