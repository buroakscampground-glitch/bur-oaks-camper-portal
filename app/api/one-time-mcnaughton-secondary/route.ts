...
const ONE_TIME_KEY = '7b0a06ec43804d1d9d25dce9add71992'
const SECONDARY_EMAIL = 'ram7045417@gmail.com'
const MARKER = 'one-time-mcnaughton-secondary-2026-09-05'
...
async function findMcNaughton(admin: any) {
  const { data, error } = await admin
    ...
    .or('last_name.ilike.%naughton%,second_profile_last_name.ilike.%naughton%')
  if (error) throw new Error(error.message)
  return (data || []).filter((row: any) => row.active !== false && String(row.role || '').toLowerCase() !== 'admin')
}
...
export async function GET(request: Request) {
  ...
  try {
    return NextResponse.json({ matches: await findMcNaughton(admin) })
  } catch (error: any) {
...
export async function POST(request: Request) {
  ...
  try {
    const matches = await findMcNaughton(admin)
    if (matches.length !== 1) {
      return NextResponse.json({ error: `Expected one active McNaughton camper record; found ${matches.length}. No change was made.`, matches }, { status: 409 })
    }
    ...
    if (currentSecondary && currentSecondary !== SECONDARY_EMAIL) {
      return NextResponse.json({ error: 'McNaughton already has a different second-profile email. No change was made.', camper }, { status: 409 })
    }
    ...
    if (conflictingOwner) {
      return NextResponse.json({ error: `That email is already attached to active Lot ${conflictingOwner.lot_number || 'unknown'}. No change was made.` }, { status: 409 })
    }
    ...
    if (currentSecondary !== SECONDARY_EMAIL) {
      const { error: updateError } = await admin.from('campers').update({ secondary_email: SECONDARY_EMAIL }).eq('id', camper.id)
