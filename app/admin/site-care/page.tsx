...
export default function AdminSiteCarePage() {
  ...
  async function sendNotice(event: React.FormEvent) {
    ...
    try {
      ...
      const response = await fetch('/api/site-care-notices', {
        ...
        body: JSON.stringify({ camperId, templateKey, title, message, dueDate, priority }),
      })
  ...
  async function changeStatus(notice: any, action: 'resolve' | 'reopen') {
    ...
    try {
      ...
      setFeedback(action === 'resolve' ? 'Resolved and moved to history.' : 'Notice moved back to the active list.')
    } catch (error: any) {
          ...
          <form className="site-care-form" onSubmit={sendNotice}>
            ...
            <label>Tap a common issue
              <div className="site-care-templates">
                ...
                <button type="button" className={templateKey === 'custom' ? 'selected' : ''} onClick={() => { setTemplateKey('custom'); setTitle(''); setMessage('') }}>Custom notice</button>
              </div>
            </label>
          ...
          <div className="site-care-list">
            {visibleNotices.map((notice) => {
              ...
              return <article className={`site-care-item ${notice.priority === 'Important' ? 'important' : ''} ${notice.status === 'Ready for Review' ? 'ready' : ''}`} key={notice.id}>
                <div className="site-care-item-top"><div><small>{notice.priority} · LOT {notice.lot_number || '—'} · {camper ? camperName(camper) : 'Camper'}</small><h3>{notice.title}</h3></div><span className="site-care-status">{notice.status}</span></div>
                <p>{notice.message}</p>
                <div className="site-care-item-actions"><span>{notice.due_date ? `Requested by ${formatDate(`${notice.due_date}T12:00:00`)}` : `Sent ${formatDate(notice.created_at)}`}</span>{notice.status === 'Resolved' ? <button type="button" disabled={updating === notice.id} onClick={() => changeStatus(notice, 'reopen')}><Eye size={15} /> Reopen</button> : <button type="button" disabled={updating === notice.id} onClick={() => changeStatus(notice, 'resolve')}><CheckCircle2 size={15} /> Mark Resolved</button>}</div>
              </article>
            })}
