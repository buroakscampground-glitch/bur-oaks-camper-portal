...
type Renewal = {
  ...
  renewal_document_id?: string | null
...
function nonRenewalPreview(camper: Camper, contractEndDate: string) {
  ...
  return [
    ...
    'As provided in the seasonal agreement, all permanent improvements and anything affixed to the ground are and remain the property of Bur Oaks Campground. This includes, without limitation, decks, patios, utility connections, rock or stone, landscaping, bushes, trees, and other plantings. These items must not be removed, altered, sold, or damaged without prior written authorization from Bur Oaks Campground.',
...
function renewalDocumentSummary(document?: RenewalDocument) {
  ...
  if (status === 'not_required') return 'Portal copy closed—the office recorded the signed renewal from the previous system.'
  return 'Unsigned—the camper has not completed or declined this renewal yet.'
}
...
export default function AdminRenewalsPage() {
  ...
  async function repairRentSchedule(camper: Camper) {
    const annualRent = Number(repairAnnualRent.replace(/[$,\s]/g, ''))
  ...
  const rows = useMemo(() => {
    ...
    return campers.map((camper) => {
      ...
      const responseDaysRemaining = renewalResponseDaysRemaining(contractEnd, today)
      ...
      return { camper, renewal, contractEnd, sendDue, reviewDue, responseDue, responseDaysRemaining, openingDate, confirmedOpening, campgroundDecision, camperDeclined, approvedToSend, nonRenewalLetterDue, nonRenewalNeedsApproval, needsReview, safe, responseOverdue, sendOverdue, sendSoon, needsSetup, automationError, needsAction, priority }
  ...
  const visibleRows = rows.filter((row) => {
    ...
    if (view === 'ToSend' && (!row.approvedToSend || row.renewal?.renewal_sent_at || row.campgroundDecision)) return false
    if (view === 'Sent' && (!row.renewal?.renewal_sent_at || row.campgroundDecision)) return false
  ...
  const renewalsToSend = rows.filter((row) => row.approvedToSend && !row.renewal?.renewal_sent_at && !row.campgroundDecision)
  const renewalsSent = rows.filter((row) => row.renewal?.renewal_sent_at && !row.campgroundDecision)
  ...
  return (
    <main className="renewal-page">
      <style>{`
        ...
        @media(max-width:680px){.site-history-panel{width:100vw}.site-history-header{padding:17px 16px}.site-history-header h2{font-size:25px}.site-history-content{padding:15px 14px 24px}.site-history-summary{grid-template-columns:1fr 1fr}.site-history-rent-repair{grid-template-columns:1fr}.site-history-rent-repair button{width:100%}.site-history-actions{display:grid;grid-template-columns:1fr 1fr}.site-history-actions>*{width:100%}.site-history-payment{align-items:start}.site-history-payment-amount strong{font-size:16px}}
      `}</style>
      ...
      <section className="renewal-summary" aria-label="Renewal overview">
        <button type="button" className={`delivery${view === 'ToSend' ? ' selected' : ''}`} onClick={() => setView('ToSend')}><span>Renewals to be sent</span><strong>{renewalsToSend.length}</strong><small>Approved and scheduled</small></button>
        <button type="button" className={`delivery sent${view === 'Sent' ? ' selected' : ''}`} onClick={() => setView('Sent')}><span>Renewals sent</span><strong>{renewalsSent.length}</strong><small>Delivered to camper portals</small></button>
      ...
      <section className="renewal-board">
        ...
        <div className="renewal-board-grid">
          ...
          <div className="renewal-bucket blue"><strong>Blue · Waiting on camper</strong><small>Renewal delivered, but not signed or declined yet</small><div className="renewal-lot-chips">{awaiting.map((row) => <button key={row.camper.id} type="button" onClick={() => openSiteHistory(row.camper)} aria-label={`Open Lot ${row.camper.lot_number || 'unassigned'} history`}>Lot {row.camper.lot_number || '—'}</button>)}{!awaiting.length && <span>None</span>}</div></div>
      ...
      <section className="renewal-roster">
        <div className="renewal-roster-head">
          <div><span className="renewal-eyebrow"><Clock3 size={15} /> RENEWAL ROSTER</span><h2>{view === 'ToSend' ? 'Renewals to be sent' : view === 'Sent' ? 'Renewals sent' : view === 'Setup' ? 'Campers needing dates' : view === 'Openings' ? 'Opening forecast details' : view === 'All' ? 'All seasonal campers' : 'Decisions needing attention'}</h2><p>{view === 'ToSend' ? 'These renewals are approved and waiting for their scheduled send date.' : view === 'Sent' ? 'These renewals were delivered to the camper portal. Signed and awaiting-response records stay visible here.' : 'Automatic renewals are protected against duplicates. You can still update any decision manually.'}</p></div>
        ...
        <div className="renewal-list">
          {visibleRows.map((row) => {
            ...
            const deadlineLabel = view === 'Sent'
              ? 'Renewal sent'
              : view === 'ToSend'
                ? 'Scheduled to send'
                : row.campgroundDecision
            ...
            const signingCountdown = row.renewal?.status === 'Awaiting Response' ? renewalResponseCountdownLabel(row.responseDaysRemaining) : ''
            return <article id={`renewal-${row.camper.id}`} className={`renewal-row${attention ? ' attention' : ''}${row.confirmedOpening ? ' opening' : ''}${row.renewal?.status === 'Awaiting Response' ? ' awaiting' : ''}`} key={row.camper.id}>
              <div className="renewal-row-main">
                ...
                <div className="renewal-deadline"><small>{deadlineLabel}</small><strong>{formatDate(deadlineDate)}</strong>{signingCountdown && <em className={row.responseOverdue ? 'overdue' : ''}>{signingCountdown}</em>}</div>
              ...
              {isExpanded && <div className="renewal-edit">
                ...
                {row.renewal?.renewal_document_id && <section className={`renewal-document-record ${String(renewalDocument?.signature_status || 'pending').toLowerCase()}`}>
                  ...
                  <a href={`/documents/view/${row.renewal.renewal_document_id}`} target="_blank" rel="noreferrer"><ExternalLink size={14} /> Open document</a>
                </section>}
        ...
        <aside className="site-history-panel" role="dialog" aria-modal="true" aria-labelledby="site-history-title">
          ...
          {!siteHistoryLoading && siteHistory && <div className="site-history-content">
            <section className="site-history-summary" aria-label="Site history summary">
              ...
              <article className={`site-history-stat${siteHistory.usage?.signal === 'no_activity' ? ' alert' : siteHistory.usage && siteHistory.usage.signal !== 'regular' ? ' warning' : ''}`}><span><Activity size={13} /> Season use</span><strong>{siteHistory.usage ? `${Math.round(siteHistory.usage.totalKwh).toLocaleString()} kWh` : 'No data'}</strong><small>{siteHistory.usage ? `${siteHistory.usage.usageBand} · ${siteHistory.usage.readingCount} reading${siteHistory.usage.readingCount === 1 ? '' : 's'}` : 'Usage baseline unavailable'}</small></article>
            </section>
            ...
            {selectedRow && <section className="site-renewal-decision" aria-label="Renewal decision">
              ...
              {siteHistory.usage && <div className={`site-history-usage-callout${siteHistory.usage.signal === 'no_activity' ? ' alert' : siteHistory.usage.signal !== 'regular' ? ' warning' : ''}`}><Activity size={17} /><div><strong>{siteHistory.usage.signalLabel} · {Math.round(siteHistory.usage.totalKwh).toLocaleString()} kWh this season</strong><p>{siteHistory.usage.signalDetail} This is decision support; the office still makes the renewal decision.</p></div></div>}
            ...
            <div className="site-history-actions">
              ...
              <a href={`/admin/camper-usage?search=${encodeURIComponent(selectedRow?.camper.lot_number || siteHistory.camper.lot_number || '')}`}><Activity size={14} /> Open usage analytics</a>
            ...
            {selectedRow && selectedRenewing && <div className="site-history-rent-repair">
              <label>Annual rent<input aria-label="Annual rent for schedule" type="number" min="1" step="0.01" value={repairAnnualRent} onChange={(event) => setRepairAnnualRent(event.target.value)} placeholder="0.00" /></label>
              <button type="button" disabled={saving === selectedRow.camper.id} onClick={() => repairRentSchedule(selectedRow.camper)}><CircleDollarSign size={14} /> {saving === selectedRow.camper.id ? 'Repairing…' : 'Repair rent schedule'}</button>
            </div>}
